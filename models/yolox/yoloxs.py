...
def myPreproc(img, input_size, swap=(2, 0, 1)):
    if len(img.shape) == 3:
...
class ValTransform:
    ...
    def __call__(self, img, res, input_size):
        img, _ = myPreproc(img, input_size, self.swap)
        if self.legacy:
            img = img[::-1, :, :].copy()
            img /= 255.0
...
def imageflow_demo(predictor, vis_folder, current_time):
    ...
    while True:
        ...
        if ret_val:
            ...
            # jpg_out = QtGui.QPixmap(_image).scaled(thesw.Label.width(), thesw.Label.height())  # 设置图片大小
            # thesw.label.setPixmap(jpg_out)  # 设置图片显示
...
#
#     model = torch.load("C:/Users/coder/PycharmProjects/dlqt_728/models11/yolox/yolox_detector.pth")
#     print("模型加载完成")
#     model.cuda()
#     # model.half()
#     model.eval()
